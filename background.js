...
// Function to inject and move the owl GIF
function injectAndMoveGif() {
  ...
  owl.style.top = '70%';         // Establish vertical position of the GIF on the screen
  owl.style.zIndex = '9999';     // Overlays GIF on top of other elements
  ...
  let position = 600; // Set starting position (vertical movement)
  
  // Controls vertical motion of the GIF
  function moveOwl() {
    position -= 2; // Move the GIF upward by 2 pixels
    owl.style.top = position + 'px'; // Update the GIF's vertical position

    // If the GIF moves off-screen at the top, reset it to the bottom
    if (position < -owl.height) {
      position = window.innerHeight; // Restart the GIF from the bottom
    }
